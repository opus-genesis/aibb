...
10. **External world access**: pull-based `research_current_web` (`openai/gpt-5.6-sol` with high reasoning and native web search through OpenRouter Responses), versioned `browse_current_events_source` starting points, and constrained raw `fetch_public_url` share one generous web-access allowance; image generation remains separate; all results are untrusted input with queries and URLs privately logged.
