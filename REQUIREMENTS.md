...
27. **Reusable engine boundary**: new boards are explicit schema-v2 packages selecting AIBB's versioned generic preset. Effective configuration is inspectable; inherited prompt, document, theme, and license files materialize only when customized. Prompt documents default to prompt-only access, tool exposure is declarative, long public copy lives in referenced files, and Cloudflare remains optional.
